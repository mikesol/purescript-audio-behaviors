...
#		-s SINGLE_FILE=1 \
#		PureScriptKernel.cc \
#		-o cpptest/pure-script-kernel.wasmmodule.js \
#		--post-js ./em-es6-module.js

#clean:
#	@rm -f pure-script-kernel.wasmmodule.js
...
DEBUG := "-DDEBUG -g4 --source-map-base"
RELEASE := "-DNDEBUG -O3"

INCLUDES := -I $(CC_SRC)
BIN_DIR := $(OUTPUT)/bin

PACKAGE_SOURCES = $(subst \,/,$(shell $(PACKAGE_MGR) -x cpptest.dhall sources))
...
$(PURESCRIPT_PKGS):
	@echo "Getting packages using" $(PACKAGE_MGR) "..."
	@$(PACKAGE_MGR) install
...
$(BIN): $(OBJS)
	@echo "Linking purescript kernel"
	@$(CXX) --bind \
	  -s WASM=1 \
		-s BINARYEN_ASYNC_COMPILATION=0 \
		-s SINGLE_FILE=1 \
